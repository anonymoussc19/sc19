...
A[+a* C_range_e* C_range_f* C_range_c+e* C_range_f* C_range_c+f* C_range_c+c] * 
B[+f* C_range_b* C_range_e* C_range_d+b* C_range_e* C_range_d+e* C_range_d+d]; 
}
...
//    Tk1 = 12;
    Tk0 = 1;
    Tm4 =  A_range_a *A_range_c;
    Tm3 = 60;
    Tm2 = 60;
    Tm1 = 6;
    Tm0 = 6;

    Tn4 = B_range_d * B_range_b;
    Tn3 = 3072;
    Tn2 = 16;
    Tn1 = 8;
    Tn0 = 8;
...
A = (double*)memalign(4096, sizeof(double) * A_range_a * A_range_e * A_range_f * A_range_c);
B = (double*)memalign(4096, sizeof(double) * B_range_f * B_range_b * B_range_e * B_range_d);
for(int i = 0; i < 1*A_range_a*A_range_e*A_range_f*A_range_c; i++){
A[i] = rand()%1000;
}
for(int i = 0; i < 1*B_range_f*B_range_b*B_range_e*B_range_d; i++){
B[i] = rand()%1000;
}
...
double alpha = 1.0;
 ...
 /* warm up */
    CPUID(); RDTSC(a); CPUID(); RDTSC(b);
        ...
        int tot_runtime = 5;
    RDTSC(a);        clock_t begin = clock();
    for(int ccnt = 0; ccnt < tot_runtime; ccnt++){


//Abuf = (double*)memalign(4096, sizeof(double) * A_range_a * A_range_e * A_range_f * A_range_c);
Abuf = (double*)memalign(4096, sizeof(double) * Tm3*Tk3);
int ext_range_c = A_range_c % Tm0;
int base_range_c = A_range_c - ext_range_c;
int base_m_range = base_range_c * A_range_a;
unsigned long long *Ascatter = (unsigned long long*)memalign(4096, sizeof(unsigned long long)*A_range_a*A_range_c);

int A_stride_a = 1 * A_range_e * A_range_f * A_range_c;
int A_stride_e = 1 * A_range_f * A_range_c;
int A_stride_f = 1 * A_range_c;
int A_stride_c = 1;

int Fused_A_stride_a = 1 * base_range_c;
int Fused_A_stride_c = 1;
int Ext_A_stride_a = 1 * ext_range_c;
int Ext_A_stride_c = 1;

for(int Im=0; Im < base_m_range;Im++){
int Ia = Im / Fused_A_stride_a;
int Ic = Im % Fused_A_stride_a;
Ascatter[Im] = +Ia*C_range_b*C_range_c*C_range_d+Ic*C_range_d;
}
for(int Im=base_m_range; Im < Tm4;Im++){
int Ia = (Im - base_m_range) / Ext_A_stride_a;
int Ic = (Im - base_m_range) % Ext_A_stride_a+ base_range_c;
Ascatter[Im] = +Ia*C_range_b*C_range_c*C_range_d+Ic*C_range_d;
}
Bbuf = (double*)memalign(4096, sizeof(double) * Tk3*Tn3);
//Bbuf = (double*)memalign(4096, sizeof(double) * B_range_f * B_range_b * B_range_e * B_range_d);
int ext_range_d = B_range_d % Tn0;
int base_range_d = B_range_d - ext_range_d;
int base_n_range = base_range_d * B_range_b;
unsigned long long *Bscatter = (unsigned long long*)memalign(4096, sizeof(unsigned long long)*B_range_b*B_range_d);

int B_stride_f = 1 * B_range_b * B_range_e * B_range_d;
int B_stride_b = 1 * B_range_e * B_range_d;
int B_stride_e = 1 * B_range_d;
int B_stride_d = 1;
...
Bscatter[In] = +Ib*C_range_c*C_range_d+Id;
}
...
/* int Ia = Im / Fused_A_stride_a; */
/* int Ic = Im % Fused_A_stride_a; */
/* Abuf[A_pack_offset] = A[ +Ia*A_stride_a +Ie*A_stride_e +If*A_stride_f +Ic*A_stride_c]; */
/* } */
/* else{ */
/* int Ia = (Im - base_m_range) / Ext_A_stride_a; */
/* int Ic = (Im - base_m_range) % Ext_A_stride_a+ base_range_c; */
...
/* int Ib = In / Fused_B_stride_b; */
/* int Id = In % Fused_B_stride_b; */
/* Bbuf[B_pack_offset] = B[ +If*B_stride_f +Ib*B_stride_b +Ie*B_stride_e +Id*B_stride_d]; */
/* } */
/* else{ */
/* int Ib = (In - base_n_range) / Ext_B_stride_b; */
/* int Id = (In - base_n_range) % Ext_B_stride_b+ base_range_d; */
