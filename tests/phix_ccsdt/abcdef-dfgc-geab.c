...
A[+g* C_range_e* C_range_a* C_range_b+e* C_range_a* C_range_b+a* C_range_b+b] * 
B[+d* C_range_f* C_range_g* C_range_c+f* C_range_g* C_range_c+g* C_range_c+c]; 
}
...
//    Tk1 = 12;
    Tk0 = 1;
    Tm4 =  A_range_b * A_range_a * A_range_e;
    Tm3 =  102;
    Tm2 = 102;
    Tm1 = 102;
    Tm0 = 6;
...
A = (double*)memalign(4096, sizeof(double) * A_range_g * A_range_e * A_range_a * A_range_b);
B = (double*)memalign(4096, sizeof(double) * B_range_d * B_range_f * B_range_g * B_range_c);
for(int i = 0; i < 1*A_range_g*A_range_e*A_range_a*A_range_b; i++){
A[i] = rand()%1000;
}
for(int i = 0; i < 1*B_range_d*B_range_f*B_range_g*B_range_c; i++){
B[i] = rand()%1000;
}
...
Abuf = (double*)memalign(4096, sizeof(double) * A_range_g * A_range_e * A_range_a * A_range_b);
int ext_range_e = A_range_e % Tm0;
int base_range_e = A_range_e - ext_range_e;
int base_m_range = base_range_e * A_range_b * A_range_a;
unsigned long long *Ascatter = (unsigned long long*)memalign(4096, sizeof(unsigned long long)*A_range_a*A_range_b*A_range_e);

int A_stride_g = 1 * A_range_e * A_range_a * A_range_b;
int A_stride_e = 1 * A_range_a * A_range_b;
int A_stride_a = 1 * A_range_b;
int A_stride_b = 1;

int Fused_A_stride_a = 1 * A_range_b * base_range_e;
int Fused_A_stride_b = 1 * base_range_e;
int Fused_A_stride_e = 1;
int Ext_A_stride_a = 1 * A_range_b * ext_range_e;
int Ext_A_stride_b = 1 * ext_range_e;
int Ext_A_stride_e = 1;

for(int Im=0; Im < base_m_range;Im++){
int Ia = Im / Fused_A_stride_a;
int Ib = Im % Fused_A_stride_a / Fused_A_stride_b;
int Ie = Im % Fused_A_stride_b;
Ascatter[Im] = +Ia*C_range_b*C_range_c*C_range_d*C_range_e*C_range_f+Ib*C_range_c*C_range_d*C_range_e*C_range_f+Ie*C_range_f;
}
for(int Im=base_m_range; Im < Tm4;Im++){
int Ia = (Im - base_m_range) / Ext_A_stride_a;
int Ib = (Im - base_m_range)  % Ext_A_stride_a / Ext_A_stride_b;
int Ie = (Im - base_m_range) % Ext_A_stride_b+ base_range_e;
Ascatter[Im] = +Ia*C_range_b*C_range_c*C_range_d*C_range_e*C_range_f+Ib*C_range_c*C_range_d*C_range_e*C_range_f+Ie*C_range_f;
}

Bbuf = (double*)memalign(4096, sizeof(double) * B_range_d * B_range_f * B_range_g * B_range_c);
int ext_range_f = B_range_f % Tn0;
int base_range_f = B_range_f - ext_range_f;
int base_n_range = base_range_f * B_range_d * B_range_c;
unsigned long long *Bscatter = (unsigned long long*)memalign(4096, sizeof(unsigned long long)*B_range_c*B_range_d*B_range_f);

int B_stride_d = 1 * B_range_f * B_range_g * B_range_c;
int B_stride_f = 1 * B_range_g * B_range_c;
int B_stride_g = 1 * B_range_c;
int B_stride_c = 1;

int Fused_B_stride_c = 1 * B_range_d * base_range_f;
int Fused_B_stride_d = 1 * base_range_f;
int Fused_B_stride_f = 1;
int Ext_B_stride_c = 1 * B_range_d * ext_range_f;
...
Bscatter[In] = +Ic*C_range_d*C_range_e*C_range_f+Id*C_range_e*C_range_f+If;
}
...
int Ib = Im % Fused_A_stride_a / Fused_A_stride_b;
int Ie = Im % Fused_A_stride_b;
Abuf[A_pack_offset] = A[ +Ig*A_stride_g +Ie*A_stride_e +Ia*A_stride_a +Ib*A_stride_b];
}
else{
int Ia = (Im - base_m_range) / Ext_A_stride_a;
int Ib = (Im - base_m_range)  % Ext_A_stride_a / Ext_A_stride_b;
...
int Id = In % Fused_B_stride_c / Fused_B_stride_d;
int If = In % Fused_B_stride_d;
Bbuf[B_pack_offset] = B[ +Id*B_stride_d +If*B_stride_f +Ig*B_stride_g +Ic*B_stride_c];
}
else{
int Ic = (In - base_n_range) / Ext_B_stride_c;
int Id = (In - base_n_range)  % Ext_B_stride_c / Ext_B_stride_d;
