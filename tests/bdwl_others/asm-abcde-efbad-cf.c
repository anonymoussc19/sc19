...
A[+c* C_range_f+f] * 
B[+e* C_range_f* C_range_b* C_range_a* C_range_d+f* C_range_b* C_range_a* C_range_d+b* C_range_a* C_range_d+a* C_range_d+d]; 
}
...
int main(int argc, char** argv){
  ...
  int Tk4, Tk3, Tk2, Tk1, Tk0;
    ...
    Tk4 = C_range_f;
    Tm4 = A_range_c;
    Tn4 = B_range_a * B_range_b * B_range_d*B_range_e;
...
A = (double*)memalign(4096, sizeof(double) * A_range_c * A_range_f);
B = (double*)memalign(4096, sizeof(double) * B_range_e * B_range_f * B_range_b * B_range_a * B_range_d);
for(int i = 0; i < 1*A_range_c*A_range_f; i++){
A[i] = rand()%1000;
}
for(int i = 0; i < 1*B_range_e*B_range_f*B_range_b*B_range_a*B_range_d; i++){
B[i] = rand()%1000;
}
...
//Abuf = (double*)memalign(4096, sizeof(double) * A_range_c * A_range_f);
...
Ascatter[Im] = +Ic*C_range_d*C_range_e;
}
...
//Bbuf = (double*)memalign(4096, sizeof(double) * B_range_e * B_range_f * B_range_b * B_range_a * B_range_d);
...
int base_n_range = base_range_e * B_range_d * B_range_b * B_range_a;
unsigned long long *Bscatter = (unsigned long long*)memalign(4096, sizeof(unsigned long long)*B_range_a*B_range_b*B_range_d*B_range_e);

int B_stride_e = 1 * B_range_f * B_range_b * B_range_a * B_range_d;
int B_stride_f = 1 * B_range_b * B_range_a * B_range_d;
int B_stride_b = 1 * B_range_a * B_range_d;
int B_stride_a = 1 * B_range_d;
int B_stride_d = 1;

int Fused_B_stride_a = 1 * B_range_b * B_range_d * base_range_e;
int Fused_B_stride_b = 1 * B_range_d * base_range_e;
int Fused_B_stride_d = 1 * base_range_e;
int Fused_B_stride_e = 1;
int Ext_B_stride_a = 1 * B_range_b * B_range_d * ext_range_e;
int Ext_B_stride_b = 1 * B_range_d * ext_range_e;
...
int Id = In % Fused_B_stride_b / Fused_B_stride_d;
int Ie = In % Fused_B_stride_d;
Bscatter[In] = +Ia*C_range_b*C_range_c*C_range_d*C_range_e+Ib*C_range_c*C_range_d*C_range_e+Id*C_range_e+Ie;
}
...
int Id = (In - base_n_range)  % Ext_B_stride_b / Ext_B_stride_d;
...
/* if(Im< base_m_range){ */
/* int Ic = Im / Fused_A_stride_c; */
/* Abuf[A_pack_offset] = A[ +Ic*A_stride_c +If*A_stride_f]; */
/* } */
/* else{ */
/* int Ic = (Im - base_m_range) / Ext_A_stride_c; */
/* Abuf[A_pack_offset] = A[ +Ic*A_stride_c +If*A_stride_f]; */
...
/* Bbuf[B_pack_offset+jj] = B[ +(Ie+jj)*B_stride_e +If*B_stride_f +Ib*B_stride_b +Ia*B_stride_a +Id*B_stride_d]; */
/* } */
/* else{ */
/* int Ia = (In - base_n_range) / Ext_B_stride_a; */
/* int Ib = (In - base_n_range)  % Ext_B_stride_a / Ext_B_stride_b; */
/* int Id = (In - base_n_range)  % Ext_B_stride_b / Ext_B_stride_d; */
/* int Ie = (In - base_n_range) % Ext_B_stride_d+ base_range_e; */
/* Bbuf[B_pack_offset] = B[ +Ie*B_stride_e +If*B_stride_f +Ib*B_stride_b +Ia*B_stride_a +Id*B_stride_d]; */
/* } */
/* // end pack body */
/* } */
/* } */
...
if(Id % 8 !=0) continue;
/* int add_Id1_pckoff = (Fused_B_stride_d %Tn1)* MIN(Tk1, Tk4-k2-k3-k4) */
/*     + (Fused_B_stride_d %Tn2 - Fused_B_stride_d %Tn1) * MIN(Tk1, Tk4-k2-k3-k4) */
/*     + (Fused_B_stride_d %Tn3 - Fused_B_stride_d %Tn2) * MIN(Tk2, Tk4-k3-k4); */

/* int add_Id2_pckoff = (2*Fused_B_stride_d %Tn1)* MIN(Tk1, Tk4-k2-k3-k4) */
/*     + (2*Fused_B_stride_d %Tn2 -2* Fused_B_stride_d %Tn1) * MIN(Tk1, Tk4-k2-k3-k4) */
/*     + (2*Fused_B_stride_d %Tn3 -2* Fused_B_stride_d %Tn2) * MIN(Tk2, Tk4-k3-k4); */

/* int add_Id3_pckoff = (3*Fused_B_stride_d %Tn1)* MIN(Tk1, Tk4-k2-k3-k4) */
/*     + (3*Fused_B_stride_d %Tn2 - 3*Fused_B_stride_d %Tn1) * MIN(Tk1, Tk4-k2-k3-k4) */
/*     + (3*Fused_B_stride_d %Tn3 - 3*Fused_B_stride_d %Tn2) * MIN(Tk2, Tk4-k3-k4); */




/* int add_Id4_pckoff = (4*Fused_B_stride_d %Tn1)* MIN(Tk1, Tk4-k2-k3-k4) */
/*     + (4*Fused_B_stride_d %Tn2 - 4*Fused_B_stride_d %Tn1) * MIN(Tk1, Tk4-k2-k3-k4) */
/*     + (4*Fused_B_stride_d %Tn3 - 4*Fused_B_stride_d %Tn2) * MIN(Tk2, Tk4-k3-k4); */

/* int add_Id5_pckoff = (5*Fused_B_stride_d %Tn1)* MIN(Tk1, Tk4-k2-k3-k4) */
/*     + (5*Fused_B_stride_d %Tn2 - 5*Fused_B_stride_d %Tn1) * MIN(Tk1, Tk4-k2-k3-k4) */
/*     + (5*Fused_B_stride_d %Tn3 - 5*Fused_B_stride_d %Tn2) * MIN(Tk2, Tk4-k3-k4); */

/* int add_Id6_pckoff = (6*Fused_B_stride_d %Tn1)* MIN(Tk1, Tk4-k2-k3-k4) */
/*     + (6*Fused_B_stride_d %Tn2 - 6*Fused_B_stride_d %Tn1) * MIN(Tk1, Tk4-k2-k3-k4) */
/*     + (6*Fused_B_stride_d %Tn3 - 6*Fused_B_stride_d %Tn2) * MIN(Tk2, Tk4-k3-k4); */

/* int add_Id7_pckoff = (7*Fused_B_stride_d %Tn1)* MIN(Tk1, Tk4-k2-k3-k4) */
/*     + (7*Fused_B_stride_d %Tn2 - 7*Fused_B_stride_d %Tn1) * MIN(Tk1, Tk4-k2-k3-k4) */
/*     + (7*Fused_B_stride_d %Tn3 - 7*Fused_B_stride_d %Tn2) * MIN(Tk2, Tk4-k3-k4); */
...
/* Bbuf[B_pack_offset+jj] = B[ +(Ie+jj)*B_stride_e +If*B_stride_f +Ib*B_stride_b +Ia*B_stride_a +Id*B_stride_d]; */
    ...
    WS_MOVE_M4xF64_TO_R4xF64((double*) &B[+(Ie + 0)*B_stride_e +If*B_stride_f +Ib*B_stride_b +Ia*B_stride_a +Id*B_stride_d  ],mymm0);

    WS_MOVE_M4xF64_TO_R4xF64((double*) &B[+(Ie + 1)*B_stride_e +If*B_stride_f +Ib*B_stride_b +Ia*B_stride_a +Id*B_stride_d  ],mymm1);

    WS_MOVE_M4xF64_TO_R4xF64((double*) &B[+(Ie + 2)*B_stride_e +If*B_stride_f +Ib*B_stride_b +Ia*B_stride_a +Id*B_stride_d  ],mymm2);

    WS_MOVE_M4xF64_TO_R4xF64((double*) &B[+(Ie + 3)*B_stride_e +If*B_stride_f +Ib*B_stride_b +Ia*B_stride_a +Id*B_stride_d  ],mymm3);

    WS_MOVE_M4xF64_TO_R4xF64((double*) &B[+(Ie + 4)*B_stride_e +If*B_stride_f +Ib*B_stride_b +Ia*B_stride_a +Id*B_stride_d  ],mymm4);

    WS_MOVE_M4xF64_TO_R4xF64((double*) &B[+(Ie + 5)*B_stride_e +If*B_stride_f +Ib*B_stride_b +Ia*B_stride_a +Id*B_stride_d  ],mymm5);

    WS_MOVE_M4xF64_TO_R4xF64((double*) &B[+(Ie + 6)*B_stride_e +If*B_stride_f +Ib*B_stride_b +Ia*B_stride_a +Id*B_stride_d  ],mymm6);

    WS_MOVE_M4xF64_TO_R4xF64((double*) &B[+(Ie + 7)*B_stride_e +If*B_stride_f +Ib*B_stride_b +Ia*B_stride_a +Id*B_stride_d  ],mymm7);
 ...
 */
    ...
    WS_MOVE_M4xF64_TO_R4xF64((double*) &B[+(Ie + 0)*B_stride_e +If*B_stride_f +Ib*B_stride_b +Ia*B_stride_a +(Id+4)*B_stride_d  ],mymm0);

    WS_MOVE_M4xF64_TO_R4xF64((double*) &B[+(Ie + 1)*B_stride_e +If*B_stride_f +Ib*B_stride_b +Ia*B_stride_a +(Id+4)*B_stride_d  ],mymm1);

    WS_MOVE_M4xF64_TO_R4xF64((double*) &B[+(Ie + 2)*B_stride_e +If*B_stride_f +Ib*B_stride_b +Ia*B_stride_a +(Id+4)*B_stride_d  ],mymm2);

    WS_MOVE_M4xF64_TO_R4xF64((double*) &B[+(Ie + 3)*B_stride_e +If*B_stride_f +Ib*B_stride_b +Ia*B_stride_a +(Id+4)*B_stride_d  ],mymm3);

    WS_MOVE_M4xF64_TO_R4xF64((double*) &B[+(Ie + 4)*B_stride_e +If*B_stride_f +Ib*B_stride_b +Ia*B_stride_a +(Id+4)*B_stride_d  ],mymm4);

    WS_MOVE_M4xF64_TO_R4xF64((double*) &B[+(Ie + 5)*B_stride_e +If*B_stride_f +Ib*B_stride_b +Ia*B_stride_a +(Id+4)*B_stride_d  ],mymm5);

    WS_MOVE_M4xF64_TO_R4xF64((double*) &B[+(Ie + 6)*B_stride_e +If*B_stride_f +Ib*B_stride_b +Ia*B_stride_a +(Id+4)*B_stride_d  ],mymm6);

    WS_MOVE_M4xF64_TO_R4xF64((double*) &B[+(Ie + 7)*B_stride_e +If*B_stride_f +Ib*B_stride_b +Ia*B_stride_a +(Id+4)*B_stride_d  ],mymm7);
...
bli_dgemm_haswell_asm_6x8(use_k,  &alpha, Abuf+ A_pack_offset, Bbuf+B_pack_offset, &beta, C+ Ascatter[Im] + Bscatter[In], 1*C_range_d*C_range_e, 1, NULL, NULL);
}
