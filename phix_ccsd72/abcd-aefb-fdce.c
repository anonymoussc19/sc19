...
A[+a* C_range_e* C_range_f* C_range_b+e* C_range_f* C_range_b+f* C_range_b+b] * 
B[+f* C_range_d* C_range_c* C_range_e+d* C_range_c* C_range_e+c* C_range_e+e]; 
}
...
A = (double*)memalign(4096, sizeof(double) * A_range_a * A_range_e * A_range_f * A_range_b);
B = (double*)memalign(4096, sizeof(double) * B_range_f * B_range_d * B_range_c * B_range_e);
for(int i = 0; i < 1*A_range_a*A_range_e*A_range_f*A_range_b; i++){
A[i] = rand()%1000;
}
for(int i = 0; i < 1*B_range_f*B_range_d*B_range_c*B_range_e; i++){
B[i] = rand()%1000;
}
...
double alpha = 1.0;
 ...
 /* warm up */
    CPUID(); RDTSC(a); CPUID(); RDTSC(b);
        ...
        int tot_runtime = 5;        clock_t begin = clock();
    RDTSC(a);
    for(int ccnt = 0; ccnt < tot_runtime; ccnt++){


//Abuf = (double*)memalign(4096, sizeof(double) * A_range_a * A_range_e * A_range_f * A_range_b);
...
int A_stride_a = 1 * A_range_e * A_range_f * A_range_b;
int A_stride_e = 1 * A_range_f * A_range_b;
int A_stride_f = 1 * A_range_b;
int A_stride_b = 1;
...
//Bbuf = (double*)memalign(4096, sizeof(double) * B_range_f * B_range_d * B_range_c * B_range_e);
...
int B_stride_f = 1 * B_range_d * B_range_c * B_range_e;
int B_stride_d = 1 * B_range_c * B_range_e;
...
/* int Ia = Im / Fused_A_stride_a; */
/* int Ib = Im % Fused_A_stride_a; */
/* Abuf[A_pack_offset] = A[ +Ia*A_stride_a +Ie*A_stride_e +If*A_stride_f +Ib*A_stride_b]; */
/* } */
/* else{ */
/* int Ia = (Im - base_m_range) / Ext_A_stride_a; */
/* int Ib = (Im - base_m_range) % Ext_A_stride_a+ base_range_b; */
...
/* int Ic = In / Fused_B_stride_c; */
/* int Id = In % Fused_B_stride_c; */
/* Bbuf[B_pack_offset] = B[ +If*B_stride_f +Id*B_stride_d +Ic*B_stride_c +Ie*B_stride_e]; */
/* } */
/* else{ */
/* int Ic = (In - base_n_range) / Ext_B_stride_c; */
/* int Id = (In - base_n_range) % Ext_B_stride_c+ base_range_d; */
