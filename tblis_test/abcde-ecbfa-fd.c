...
//    a4=b4=d4=312; c4=24;
    ...
    f4 = C_range_f;
//cgab, gefd
    double* data_A = (double*)memalign(4096,d4*f4*sizeof(double));
    tblis_tensor A;


    len_type Altp[2] = { f4,d4}; // m*k
    stride_type Astp[2] = { d4, 1};
    len_type* Alentp = &Altp;

    stride_type*  Asttp = &Astp;
    for(int i=0;i<f4*d4;i++)data_A[i] = rand()%1000;


    tblis_init_tensor_d(&A, 2, Alentp, data_A, Asttp);

    double* data_B = (double*)memalign(4096,e4*c4*b4*f4*a4*sizeof(double));
    tblis_tensor B;
    len_type Bltp[5] = {e4, c4,b4,f4,a4};// n*k

    
//    stride_type Bstp[4] = {e4*f4*d4, f4*d4, d4,1};
    stride_type Bstp[5] = {c4*b4*f4*a4, b4*f4*a4, a4*f4, a4,1};
    len_type* Blentp = &Bltp;
    stride_type*  Bsttp = &Bstp;
    for(int i=0;i<e4*c4*b4*f4*a4;i++)data_B[i] = rand()%1000;
...
//    tblis_tensor_mult(tblis_single, NULL, &A, "cgab", &B, "gefd", &C, "abcdef");
    ...
    tblis_tensor_mult(tblis_single, NULL, &A, "fd", &B, "ecbfa", &C, "abcde");
//        tblis_tensor_mult(tblis_single, NULL, &A, "cgab", &B, "gefd", &C, "abcdef");
