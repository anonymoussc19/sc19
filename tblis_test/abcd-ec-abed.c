...
//cgab, gefd
    double* data_A = (double*)memalign(4096,c4*e4*sizeof(double));
    tblis_tensor A;


    len_type Altp[2] = {e4, c4}; // m*k
    stride_type Astp[2] = {   c4, 1};
    len_type* Alentp = &Altp;

    stride_type*  Asttp = &Astp;
    for(int i=0;i<c4*e4;i++)data_A[i] = rand()%1000;


    tblis_init_tensor_d(&A, 2, Alentp, data_A, Asttp);

    double* data_B = (double*)memalign(4096,a4*e4*b4*d4*sizeof(double));
    tblis_tensor B;
    len_type Bltp[4] = {a4,e4,b4,d4 };// n*k

    
//    stride_type Bstp[4] = {e4*f4*d4, f4*d4, d4,1};
    stride_type Bstp[4] = {e4*b4*d4, e4*d4,d4,1};
    len_type* Blentp = &Bltp;
    stride_type*  Bsttp = &Bstp;
    for(int i=0;i<d4*b4*e4*a4;i++)data_B[i] = rand()%1000;
...
//    tblis_tensor_mult(tblis_single, NULL, &A, "cgab", &B, "gefd", &C, "abcdef");
    ...
    tblis_tensor_mult(tblis_single, NULL, &A, "ec", &B, "abed", &C, "abcd");
//        tblis_tensor_mult(tblis_single, NULL, &A, "cgab", &B, "gefd", &C, "abcdef");
