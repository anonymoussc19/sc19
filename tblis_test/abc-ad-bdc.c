...
//    a4=b4=d4=312; c4=24;
    ...
    d4 = C_range_d;
//cgab, gefd
    double* data_A = (double*)memalign(4096,a4*d4*sizeof(double));
    tblis_tensor A;


    len_type Altp[2] = {a4,d4,}; // m*k
    stride_type Astp[2] = {   d4, 1};
    len_type* Alentp = &Altp;

    stride_type*  Asttp = &Astp;
    for(int i=0;i<a4*d4;i++)data_A[i] = rand()%1000;
    ...
    tblis_tensor B;
    len_type Bltp[3] = {b4,d4, c4 };// n*k

    
//    stride_type Bstp[4] = {e4*f4*d4, f4*d4, d4,1};
    stride_type Bstp[3] = {c4*d4,c4,1};
...
//    tblis_tensor_mult(tblis_single, NULL, &A, "cgab", &B, "gefd", &C, "abcdef");
    ...
    tblis_tensor_mult(tblis_single, NULL, &A, "ad", &B, "bdc", &C, "abc");
//        tblis_tensor_mult(tblis_single, NULL, &A, "cgab", &B, "gefd", &C, "abcdef");
