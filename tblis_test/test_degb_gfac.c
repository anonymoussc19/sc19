...
//    a4=c4=19; f4=d4=;
    ...
    for(int g = 0; g<g4;g++){
        ...
        C[a* b4*c4*d4*e4*f4 +
          ...
          f]  +=
//dega gfbc
            A[d *e4*g4*b4 + e *g4*b4 + g* b4+b *1]
            *
            B[g* f4*a4*c4 + f *a4*c4 + a*c4+ c];
...
//    int m=3072, n=3072, k=3072;
    ...
    double* data_A = (double*)memalign(4096,d4*e4*g4*b4*sizeof(double));
    tblis_tensor A;
    len_type Altp[4] = {d4,e4,g4,b4}; // m*k
    stride_type Astp[4] = {e4*g4*b4 , g4*b4 ,b4 ,1};
    len_type* Alentp = &Altp;
    stride_type*  Asttp = &Astp;
    for(int i=0;i<d4*e4*g4*b4;i++)data_A[i] = rand()/1000;
    ...
    double* data_B = (double*)memalign(4096,g4*f4*a4*c4*sizeof(double));
    tblis_tensor B;
    len_type Bltp[4] = {g4,f4,a4,c4 };// n*k
    stride_type Bstp[4] = {a4*f4*c4, a4*c4, c4,1};
    len_type* Blentp = &Bltp;
    stride_type*  Bsttp = &Bstp;
    for(int i=0;i<g4*f4*a4*c4;i++)data_B[i] = rand()/1000;
...
//    tblis_tensor_mult(tblis_single, NULL, &A, "cgab", &B, "gefd", &C, "abcdef");
    ...
    tblis_tensor_mult(tblis_single, NULL, &A, "degb", &B, "gfac", &C, "abcdef");
//        tblis_tensor_mult(tblis_single, NULL, &A, "cgab", &B, "gefd", &C, "abcdef");
