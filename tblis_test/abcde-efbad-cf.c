...
//    a4=b4=d4=312; c4=24;
    ...
    f4 = C_range_f;
//cgab, gefd
    double* data_A = (double*)memalign(4096,c4*f4*sizeof(double));
    tblis_tensor A;


    len_type Altp[2] = { c4,f4}; // m*k
    stride_type Astp[2] = { f4, 1};
    len_type* Alentp = &Altp;

    stride_type*  Asttp = &Astp;
    for(int i=0;i<f4*c4;i++)data_A[i] = rand()%1000;


    tblis_init_tensor_d(&A, 2, Alentp, data_A, Asttp);

    double* data_B = (double*)memalign(4096,e4*f4*b4*a4*d4*sizeof(double));
    tblis_tensor B;
    len_type Bltp[5] = {e4, f4,b4,a4,d4};// n*k

    
//    stride_type Bstp[4] = {e4*f4*d4, f4*d4, d4,1};
    stride_type Bstp[5] = {f4*b4*a4*d4, b4*a4*d4, a4*d4, d4,1};
    len_type* Blentp = &Bltp;
    stride_type*  Bsttp = &Bstp;
    for(int i=0;i<e4*f4*b4*a4*d4;i++)data_B[i] = rand()%1000;
    tblis_init_tensor_d(&B, 5, Blentp, data_B, Bsttp);

    double* data_C = (double*)memalign(4096,a4*b4*c4*d4*e4*sizeof(double));
    double* data_C2 = (double*)memalign(4096,a4*b4*c4*d4*e4*sizeof(double));
    for(int i = 0; i < a4*b4*c4*d4*e4; i++){
        data_C[i] = data_C2[i]=0;
    }

    tblis_tensor C;
    len_type Cltp[5] = {a4,b4,c4,d4,e4}; // m*n
//    stride_type Cstp[6] = {24*24*24*24*24, 24*24*24*24,24*24*24,24*24,24,1};
    stride_type Cstp[5] = {b4*c4*d4*e4,c4*d4*e4, d4*e4,e4, 1};
    len_type* Clentp = &Cltp;
    stride_type*  Csttp =&Cstp;

    tblis_init_tensor_d(&C, 5, Clentp, data_C, Csttp);
...
//    tblis_tensor_mult(tblis_single, NULL, &A, "cgab", &B, "gefd", &C, "abcdef");
    ...
    tblis_tensor_mult(tblis_single, NULL, &A, "cf", &B, "efbad", &C, "abcde");
//        tblis_tensor_mult(tblis_single, NULL, &A, "cgab", &B, "gefd", &C, "abcdef");
...
//        printf ("cnt\n");
    ...
    double flop_count = 2.0*a4*b4*c4*d4*e4*f4*ccnt;
