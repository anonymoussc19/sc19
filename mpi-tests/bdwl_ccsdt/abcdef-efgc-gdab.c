...
A[+g* C_range_d* C_range_a* C_range_b+d* C_range_a* C_range_b+a* C_range_b+b] * 
B[+e* C_range_f* C_range_g* C_range_c+f* C_range_g* C_range_c+g* C_range_c+c]; 
}
...
//    Tk1 = 12;
    Tk0 = 1;
    Tm4 =  A_range_b * A_range_a * A_range_d;
    ...
    Tn4 = B_range_c * B_range_e * B_range_f;
                Tn3 = Tn4;
    Tn2 = 16;
    Tn1 = 8;
...
A = (double*)memalign(4096, sizeof(double) * A_range_g * A_range_d * A_range_a * A_range_b);
B = (double*)memalign(4096, sizeof(double) * B_range_e * B_range_f * B_range_g * B_range_c);
for(int i = 0; i < 1*A_range_g*A_range_d*A_range_a*A_range_b; i++){
A[i] = rand()%1000;
}
for(int i = 0; i < 1*B_range_e*B_range_f*B_range_g*B_range_c; i++){
B[i] = rand()%1000;
}
...
Abuf = (double*)memalign(4096, sizeof(double) * A_range_g * A_range_d * A_range_a * A_range_b);
int ext_range_d = A_range_d % Tm0;
int base_range_d = A_range_d - ext_range_d;
int base_m_range = base_range_d * A_range_b * A_range_a;
unsigned long long *Ascatter = (unsigned long long*)memalign(4096, sizeof(unsigned long long)*A_range_a*A_range_b*A_range_d);

int A_stride_g = 1 * A_range_d * A_range_a * A_range_b;
int A_stride_d = 1 * A_range_a * A_range_b;
int A_stride_a = 1 * A_range_b;
int A_stride_b = 1;

int Fused_A_stride_a = 1 * A_range_b * base_range_d;
int Fused_A_stride_b = 1 * base_range_d;
int Fused_A_stride_d = 1;
int Ext_A_stride_a = 1 * A_range_b * ext_range_d;
int Ext_A_stride_b = 1 * ext_range_d;
int Ext_A_stride_d = 1;

for(int Im=0; Im < base_m_range;Im++){
int Ia = Im / Fused_A_stride_a;
int Ib = Im % Fused_A_stride_a / Fused_A_stride_b;
int Id = Im % Fused_A_stride_b;
Ascatter[Im] = +Ia*C_range_b*C_range_c*C_range_d*C_range_e*C_range_f+Ib*C_range_c*C_range_d*C_range_e*C_range_f+Id*C_range_e*C_range_f;
}
...
int Id = (Im - base_m_range) % Ext_A_stride_b+ base_range_d;
Ascatter[Im] = +Ia*C_range_b*C_range_c*C_range_d*C_range_e*C_range_f+Ib*C_range_c*C_range_d*C_range_e*C_range_f+Id*C_range_e*C_range_f;
}

Bbuf = (double*)memalign(4096, sizeof(double) * B_range_e * B_range_f * B_range_g * B_range_c);
int ext_range_f = B_range_f % Tn0;
int base_range_f = B_range_f - ext_range_f;
int base_n_range = base_range_f * B_range_e * B_range_c;
unsigned long long *Bscatter = (unsigned long long*)memalign(4096, sizeof(unsigned long long)*B_range_c*B_range_e*B_range_f);

int B_stride_e = 1 * B_range_f * B_range_g * B_range_c;
int B_stride_f = 1 * B_range_g * B_range_c;
int B_stride_g = 1 * B_range_c;
int B_stride_c = 1;

int Fused_B_stride_c = 1 * B_range_e * base_range_f;
int Fused_B_stride_e = 1 * base_range_f;
int Fused_B_stride_f = 1;
int Ext_B_stride_c = 1 * B_range_e * ext_range_f;
int Ext_B_stride_e = 1 * ext_range_f;
int Ext_B_stride_f = 1;

for(int In=0; In < base_n_range;In++){
int Ic = In / Fused_B_stride_c;
int Ie = In % Fused_B_stride_c / Fused_B_stride_e;
int If = In % Fused_B_stride_e;
Bscatter[In] = +Ic*C_range_d*C_range_e*C_range_f+Ie*C_range_f+If;
}
for(int In=base_n_range; In < Tn4;In++){
int Ic = (In - base_n_range) / Ext_B_stride_c;
int Ie = (In - base_n_range)  % Ext_B_stride_c / Ext_B_stride_e;
...
int Ib = Im % Fused_A_stride_a / Fused_A_stride_b;
int Id = Im % Fused_A_stride_b;
Abuf[A_pack_offset] = A[ +Ig*A_stride_g +Id*A_stride_d +Ia*A_stride_a +Ib*A_stride_b];
}
else{
int Ia = (Im - base_m_range) / Ext_A_stride_a;
int Ib = (Im - base_m_range)  % Ext_A_stride_a / Ext_A_stride_b;
...
int Ie = In % Fused_B_stride_c / Fused_B_stride_e;
int If = In % Fused_B_stride_e;
Bbuf[B_pack_offset] = B[ +Ie*B_stride_e +If*B_stride_f +Ig*B_stride_g +Ic*B_stride_c];
}
else{
int Ic = (In - base_n_range) / Ext_B_stride_c;
int Ie = (In - base_n_range)  % Ext_B_stride_c / Ext_B_stride_e;
