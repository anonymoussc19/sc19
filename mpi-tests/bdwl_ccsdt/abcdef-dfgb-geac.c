...
A[+g* C_range_e* C_range_a* C_range_c+e* C_range_a* C_range_c+a* C_range_c+c] * 
B[+d* C_range_f* C_range_g* C_range_b+f* C_range_g* C_range_b+g* C_range_b+b]; 
}
...
//    Tk1 = 12;
    Tk0 = 1;
    Tm4 =  A_range_c * A_range_a * A_range_e;
            Tm3 =  24;//;A_range_a * A_range_d * A_range_e;
    Tm2 = 24;
    Tm1 = 24;
    Tm0 = 6;

    Tn4 = B_range_b * B_range_d * B_range_f;
            Tn3 = Tn4;
    Tn2 = 16;
    Tn1 = 8;
    Tn0 = 8;
...
A = (double*)memalign(4096, sizeof(double) * A_range_g * A_range_e * A_range_a * A_range_c);
B = (double*)memalign(4096, sizeof(double) * B_range_d * B_range_f * B_range_g * B_range_b);
for(int i = 0; i < 1*A_range_g*A_range_e*A_range_a*A_range_c; i++){
A[i] = rand()%1000;
}
for(int i = 0; i < 1*B_range_d*B_range_f*B_range_g*B_range_b; i++){
B[i] = rand()%1000;
}
...
Abuf = (double*)memalign(4096, sizeof(double) * A_range_g * A_range_e * A_range_a * A_range_c);
int ext_range_e = A_range_e % Tm0;
int base_range_e = A_range_e - ext_range_e;
int base_m_range = base_range_e * A_range_c * A_range_a;
unsigned long long *Ascatter = (unsigned long long*)memalign(4096, sizeof(unsigned long long)*A_range_a*A_range_c*A_range_e);

int A_stride_g = 1 * A_range_e * A_range_a * A_range_c;
int A_stride_e = 1 * A_range_a * A_range_c;
int A_stride_a = 1 * A_range_c;
int A_stride_c = 1;

int Fused_A_stride_a = 1 * A_range_c * base_range_e;
int Fused_A_stride_c = 1 * base_range_e;
int Fused_A_stride_e = 1;
int Ext_A_stride_a = 1 * A_range_c * ext_range_e;
...
Ascatter[Im] = +Ia*C_range_b*C_range_c*C_range_d*C_range_e*C_range_f+Ic*C_range_d*C_range_e*C_range_f+Ie*C_range_f;
}
...
Bbuf = (double*)memalign(4096, sizeof(double) * B_range_d * B_range_f * B_range_g * B_range_b);
int ext_range_f = B_range_f % Tn0;
int base_range_f = B_range_f - ext_range_f;
int base_n_range = base_range_f * B_range_d * B_range_b;
unsigned long long *Bscatter = (unsigned long long*)memalign(4096, sizeof(unsigned long long)*B_range_b*B_range_d*B_range_f);

int B_stride_d = 1 * B_range_f * B_range_g * B_range_b;
int B_stride_f = 1 * B_range_g * B_range_b;
int B_stride_g = 1 * B_range_b;
int B_stride_b = 1;

int Fused_B_stride_b = 1 * B_range_d * base_range_f;
int Fused_B_stride_d = 1 * base_range_f;
int Fused_B_stride_f = 1;
int Ext_B_stride_b = 1 * B_range_d * ext_range_f;
...
Bscatter[In] = +Ib*C_range_c*C_range_d*C_range_e*C_range_f+Id*C_range_e*C_range_f+If;
}
...
int Ic = Im % Fused_A_stride_a / Fused_A_stride_c;
int Ie = Im % Fused_A_stride_c;
Abuf[A_pack_offset] = A[ +Ig*A_stride_g +Ie*A_stride_e +Ia*A_stride_a +Ic*A_stride_c];
}
else{
int Ia = (Im - base_m_range) / Ext_A_stride_a;
int Ic = (Im - base_m_range)  % Ext_A_stride_a / Ext_A_stride_c;
...
int Id = In % Fused_B_stride_b / Fused_B_stride_d;
int If = In % Fused_B_stride_d;
Bbuf[B_pack_offset] = B[ +Id*B_stride_d +If*B_stride_f +Ig*B_stride_g +Ib*B_stride_b];
}
else{
int Ib = (In - base_n_range) / Ext_B_stride_b;
int Id = (In - base_n_range)  % Ext_B_stride_b / Ext_B_stride_d;
