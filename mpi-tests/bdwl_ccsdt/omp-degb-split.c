...
A[+d* C_range_e* C_range_g* C_range_b+e* C_range_g* C_range_b+g* C_range_b+b] * 
B[+g* C_range_f* C_range_a* C_range_c+f* C_range_a* C_range_c+a* C_range_c+c]; 
}
...
//    Tk1 = 12;
    ...
    Tm1 = 24;
    Tk0 = 1;
    Tm4 =  A_range_b * A_range_d * A_range_e;

    
    Tm0 = 6;

    Tn4 = B_range_a * B_range_c * B_range_f;
        Tn3 = Tn4;
    Tn2 = 16;
    Tn1 = 8;
...
A = (double*)memalign(4096, sizeof(double) * A_range_d * A_range_e * A_range_g * A_range_b);
B = (double*)memalign(4096, sizeof(double) * B_range_g * B_range_f * B_range_a * B_range_c);
for(int i = 0; i < 1*A_range_d*A_range_e*A_range_g*A_range_b; i++){
A[i] = rand()%1000;
}
for(int i = 0; i < 1*B_range_g*B_range_f*B_range_a*B_range_c; i++){
B[i] = rand()%1000;
}
...
//Abuf = (double*)memalign(4096, sizeof(double) * A_range_d * A_range_e * A_range_g * A_range_b);
        Abuf = (double*)memalign(4096, sizeof(double) * Tk3*Tm3 * MYTHREAD);
int ext_range_e = A_range_e % Tm0;
int base_range_e = A_range_e - ext_range_e;
int base_m_range = base_range_e * A_range_d * A_range_b;
unsigned long long *Ascatter = (unsigned long long*)memalign(4096, sizeof(unsigned long long)*A_range_b*A_range_d*A_range_e);

int A_stride_d = 1 * A_range_e * A_range_g * A_range_b;
int A_stride_e = 1 * A_range_g * A_range_b;
int A_stride_g = 1 * A_range_b;
int A_stride_b = 1;

int Fused_A_stride_b = 1 * A_range_d * base_range_e;
int Fused_A_stride_d = 1 * base_range_e;
int Fused_A_stride_e = 1;
int Ext_A_stride_b = 1 * A_range_d * ext_range_e;
int Ext_A_stride_d = 1 * ext_range_e;
int Ext_A_stride_e = 1;

for(int Im=0; Im < base_m_range;Im++){
int Ib = Im / Fused_A_stride_b;
int Id = Im % Fused_A_stride_b / Fused_A_stride_d;
int Ie = Im % Fused_A_stride_d;
Ascatter[Im] = +Ib*C_range_c*C_range_d*C_range_e*C_range_f+Id*C_range_e*C_range_f+Ie*C_range_f;
}
for(int Im=base_m_range; Im < Tm4;Im++){
int Ib = (Im - base_m_range) / Ext_A_stride_b;
int Id = (Im - base_m_range)  % Ext_A_stride_b / Ext_A_stride_d;
int Ie = (Im - base_m_range) % Ext_A_stride_d+ base_range_e;
Ascatter[Im] = +Ib*C_range_c*C_range_d*C_range_e*C_range_f+Id*C_range_e*C_range_f+Ie*C_range_f;
}

//Bbuf = (double*)memalign(4096, sizeof(double) * B_range_g * B_range_f * B_range_a * B_range_c);
Bbuf = (double*)memalign(4096, Tn3*Tk3*sizeof(double) );
int ext_range_f = B_range_f % Tn0;
int base_range_f = B_range_f - ext_range_f;
int base_n_range = base_range_f * B_range_c * B_range_a;
unsigned long long *Bscatter = (unsigned long long*)memalign(4096, sizeof(unsigned long long)*B_range_a*B_range_c*B_range_f);

int B_stride_g = 1 * B_range_f * B_range_a * B_range_c;
int B_stride_f = 1 * B_range_a * B_range_c;
int B_stride_a = 1 * B_range_c;
int B_stride_c = 1;

int Fused_B_stride_a = 1 * B_range_c * base_range_f;
int Fused_B_stride_c = 1 * base_range_f;
int Fused_B_stride_f = 1;
int Ext_B_stride_a = 1 * B_range_c * ext_range_f;
...
Bscatter[In] = +Ia*C_range_b*C_range_c*C_range_d*C_range_e*C_range_f+Ic*C_range_d*C_range_e*C_range_f+If;
}
...
{
if(+k3+k4>=Tk4)break;
#pragma omp parallel for num_threads(MYTHREAD) schedule (dynamic)
...
int Ic = In % Fused_B_stride_a / Fused_B_stride_c;
int If = In % Fused_B_stride_c;
Bbuf[B_pack_offset] = B[ +Ig*B_stride_g +If*B_stride_f +Ia*B_stride_a +Ic*B_stride_c];
}
else{
int Ia = (In - base_n_range) / Ext_B_stride_a;
int Ic = (In - base_n_range)  % Ext_B_stride_a / Ext_B_stride_c;
...
int A_pack_offset = tid * Tm3 * Tk3
    + m0 + k1* Tm0 + m1* MIN(Tk1, Tk4-k2-k3-k4) + m2* MIN(Tk1, Tk4-k2-k3-k4) + k2* MIN(Tm2, Tm4-m3-m4) + m3* MIN(Tk2, Tk4-k3-k4) + k3* MIN(Tm3, Tm4-m4) ;//+ m4* MIN(Tk3, Tk4-k4) + k4* MIN(Tm4, Tm4);
if(Im< base_m_range){
int Ib = Im / Fused_A_stride_b;
int Id = Im % Fused_A_stride_b / Fused_A_stride_d;
int Ie = Im % Fused_A_stride_d;
Abuf[A_pack_offset] = A[ +Id*A_stride_d +Ie*A_stride_e +Ig*A_stride_g +Ib*A_stride_b];
}
else{
int Ib = (Im - base_m_range) / Ext_A_stride_b;
int Id = (Im - base_m_range)  % Ext_A_stride_b / Ext_A_stride_d;
