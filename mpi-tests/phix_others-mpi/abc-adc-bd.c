...
A[+b* C_range_d+d] * 
B[+a* C_range_d* C_range_c+d* C_range_c+c]; 
}
...
B = (double*)memalign(4096, sizeof(double) * B_range_a * B_range_d * B_range_c);
for(int i = 0; i < 1*A_range_b*A_range_d; i++){
A[i] = rand()%1000;
}
for(int i = 0; i < 1*B_range_a*B_range_d*B_range_c; i++){
B[i] = rand()%1000;
}
...
//Bbuf = (double*)memalign(4096, sizeof(double) * B_range_a * B_range_d * B_range_c);
...
int B_stride_a = 1 * B_range_d * B_range_c;
...
Bbuf[B_pack_offset+jj] = B[ +Ia*B_stride_a +Id*B_stride_d +(Ic+jj)*B_stride_c];
}
else{
int Ia = (In - base_n_range) / Ext_B_stride_a;
int Ic = (In - base_n_range) % Ext_B_stride_a+ base_range_c;
Bbuf[B_pack_offset] = B[ +Ia*B_stride_a +Id*B_stride_d +Ic*B_stride_c];
}
// end pack body
}
}
...
Abuf[A_pack_offset+ii] = A[ +(Ib+ii)*A_stride_b +Id*A_stride_d];
}
else{
int Ib = (Im - base_m_range) / Ext_A_stride_b;
