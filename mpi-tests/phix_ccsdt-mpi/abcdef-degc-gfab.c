...
A[+d* C_range_e* C_range_g* C_range_c+e* C_range_g* C_range_c+g* C_range_c+c] * 
B[+g* C_range_f* C_range_a* C_range_b+f* C_range_a* C_range_b+a* C_range_b+b]; 
}
...
//    Tk1 = 12;
    Tk0 = 1;
    Tm4 =  A_range_c * A_range_d * A_range_e;
    Tm3 =  6;
    Tm2 = 6;
    Tm1 = 6;
    Tm0 = 6;

    Tn4 = B_range_a * B_range_b * B_range_f;
    Tn3 = Tn4;
    Tn2 = 680;
    Tn1 = 8;
    Tn0 = 8;
...
A = (double*)memalign(4096, sizeof(double) * A_range_d * A_range_e * A_range_g * A_range_c);
B = (double*)memalign(4096, sizeof(double) * B_range_g * B_range_f * B_range_a * B_range_b);
for(int i = 0; i < 1*A_range_d*A_range_e*A_range_g*A_range_c; i++){
A[i] = rand()%1000;
}
for(int i = 0; i < 1*B_range_g*B_range_f*B_range_a*B_range_b; i++){
B[i] = rand()%1000;
}
...
Abuf = (double*)memalign(4096, sizeof(double) * A_range_d * A_range_e * A_range_g * A_range_c);
int ext_range_e = A_range_e % Tm0;
int base_range_e = A_range_e - ext_range_e;
int base_m_range = base_range_e * A_range_d * A_range_c;
unsigned long long *Ascatter = (unsigned long long*)memalign(4096, sizeof(unsigned long long)*A_range_c*A_range_d*A_range_e);

int A_stride_d = 1 * A_range_e * A_range_g * A_range_c;
int A_stride_e = 1 * A_range_g * A_range_c;
int A_stride_g = 1 * A_range_c;
int A_stride_c = 1;

int Fused_A_stride_c = 1 * A_range_d * base_range_e;
int Fused_A_stride_d = 1 * base_range_e;
int Fused_A_stride_e = 1;
int Ext_A_stride_c = 1 * A_range_d * ext_range_e;
int Ext_A_stride_d = 1 * ext_range_e;
int Ext_A_stride_e = 1;

for(int Im=0; Im < base_m_range;Im++){
int Ic = Im / Fused_A_stride_c;
int Id = Im % Fused_A_stride_c / Fused_A_stride_d;
int Ie = Im % Fused_A_stride_d;
Ascatter[Im] = +Ic*C_range_d*C_range_e*C_range_f+Id*C_range_e*C_range_f+Ie*C_range_f;
}
for(int Im=base_m_range; Im < Tm4;Im++){
int Ic = (Im - base_m_range) / Ext_A_stride_c;
int Id = (Im - base_m_range)  % Ext_A_stride_c / Ext_A_stride_d;
int Ie = (Im - base_m_range) % Ext_A_stride_d+ base_range_e;
Ascatter[Im] = +Ic*C_range_d*C_range_e*C_range_f+Id*C_range_e*C_range_f+Ie*C_range_f;
}

Bbuf = (double*)memalign(4096, sizeof(double) * B_range_g * B_range_f * B_range_a * B_range_b);
int ext_range_f = B_range_f % Tn0;
int base_range_f = B_range_f - ext_range_f;
int base_n_range = base_range_f * B_range_b * B_range_a;
unsigned long long *Bscatter = (unsigned long long*)memalign(4096, sizeof(unsigned long long)*B_range_a*B_range_b*B_range_f);

int B_stride_g = 1 * B_range_f * B_range_a * B_range_b;
int B_stride_f = 1 * B_range_a * B_range_b;
int B_stride_a = 1 * B_range_b;
int B_stride_b = 1;

int Fused_B_stride_a = 1 * B_range_b * base_range_f;
int Fused_B_stride_b = 1 * base_range_f;
int Fused_B_stride_f = 1;
int Ext_B_stride_a = 1 * B_range_b * ext_range_f;
int Ext_B_stride_b = 1 * ext_range_f;
int Ext_B_stride_f = 1;

for(int In=0; In < base_n_range;In++){
int Ia = In / Fused_B_stride_a;
int Ib = In % Fused_B_stride_a / Fused_B_stride_b;
int If = In % Fused_B_stride_b;
Bscatter[In] = +Ia*C_range_b*C_range_c*C_range_d*C_range_e*C_range_f+Ib*C_range_c*C_range_d*C_range_e*C_range_f+If;
}
...
int If = (In - base_n_range) % Ext_B_stride_b+ base_range_f;
Bscatter[In] = +Ia*C_range_b*C_range_c*C_range_d*C_range_e*C_range_f+Ib*C_range_c*C_range_d*C_range_e*C_range_f+If;
}
...
int Id = Im % Fused_A_stride_c / Fused_A_stride_d;
int Ie = Im % Fused_A_stride_d;
Abuf[A_pack_offset] = A[ +Id*A_stride_d +Ie*A_stride_e +Ig*A_stride_g +Ic*A_stride_c];
}
else{
int Ic = (Im - base_m_range) / Ext_A_stride_c;
int Id = (Im - base_m_range)  % Ext_A_stride_c / Ext_A_stride_d;
...
int Ib = In % Fused_B_stride_a / Fused_B_stride_b;
int If = In % Fused_B_stride_b;
Bbuf[B_pack_offset] = B[ +Ig*B_stride_g +If*B_stride_f +Ia*B_stride_a +Ib*B_stride_b];
}
else{
int Ia = (In - base_n_range) / Ext_B_stride_a;
int Ib = (In - base_n_range)  % Ext_B_stride_a / Ext_B_stride_b;
