...
A[+e* C_range_c+c] * 
B[+a* C_range_b* C_range_e* C_range_d+b* C_range_e* C_range_d+e* C_range_d+d]; 
}
...
B = (double*)memalign(4096, sizeof(double) * B_range_a * B_range_b * B_range_e * B_range_d);
for(int i = 0; i < 1*A_range_e*A_range_c; i++){
A[i] = rand()%1000;
}
for(int i = 0; i < 1*B_range_a*B_range_b*B_range_e*B_range_d; i++){
B[i] = rand()%1000;
}
...
//Bbuf = (double*)memalign(4096, sizeof(double) * B_range_a * B_range_b * B_range_e * B_range_d);
...
int B_stride_a = 1 * B_range_b * B_range_e * B_range_d;
...
/* int Ib = In % Fused_B_stride_a / Fused_B_stride_b; */
/* int Id = In % Fused_B_stride_b; */
/* Bbuf[B_pack_offset] = B[ +Ia*B_stride_a +Ib*B_stride_b +Ie*B_stride_e +Id*B_stride_d]; */
/* } */
/* else{ */
/* int Ia = (In - base_n_range) / Ext_B_stride_a; */
/* int Ib = (In - base_n_range)  % Ext_B_stride_a / Ext_B_stride_b; */
...
for(int jj = 0; jj< 8;jj++){
    Bbuf[B_pack_offset+jj] = B[ +Ia*B_stride_a +Ib*B_stride_b +Ie*B_stride_e +(jj+Id)*B_stride_d];
}
}
}
else{
...
bli_dgemm_haswell_asm_6x8(Tk3,  &alpha, Abuf+ A_pack_offset, Bbuf+B_pack_offset, &beta, C+ Ascatter[Im] + Bscatter[In], 1*C_range_d, 1, NULL, NULL);
}
