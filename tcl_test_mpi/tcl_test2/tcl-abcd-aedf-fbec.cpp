...
int main(int argc, char** argv)
{
   ...
   posix_memalign((void**) &dataA, 4096, sizeof(double) * ((size_t)tf)*td*te*ta);
   posix_memalign((void**) &dataB, 4096, sizeof(double) * ((size_t)te)*tc*tf*tb);
   ...
   tcl::Tensor<double> A({tf,td,te,ta}, dataA);
   tcl::Tensor<double> B({tc,te,tb,tf}, dataB);
...
MPI_Barrier(MPI_COMM_WORLD);  double start = omp_get_wtime();
   ...
   auto err = tcl::tensorMult<double>( alpha, A["tf,td,te,ta"], B["tc,te,tb,tf"], beta, C["td,tc,tb,ta"] );
      if( err != tcl::SUCCESS ){
