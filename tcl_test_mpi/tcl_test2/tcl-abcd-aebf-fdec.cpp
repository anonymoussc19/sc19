...
int main(int argc, char** argv)
{
   ...
   posix_memalign((void**) &dataA, 4096, sizeof(double) * ((size_t)tf)*tb*te*ta);
   posix_memalign((void**) &dataB, 4096, sizeof(double) * ((size_t)te)*tc*tf*td);
   ...
   tcl::Tensor<double> A({tf,tb,te,ta}, dataA);
   tcl::Tensor<double> B({tc,te,td,tf}, dataB);
...
MPI_Barrier(MPI_COMM_WORLD);  double start = omp_get_wtime();
   ...
   auto err = tcl::tensorMult<double>( alpha, A["tf,tb,te,ta"], B["tc,te,td,tf"], beta, C["td,tc,tb,ta"] );
      if( err != tcl::SUCCESS ){
