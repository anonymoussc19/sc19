...
int main(int argc, char** argv)
{
   ...
   posix_memalign((void**) &dataA, 4096, sizeof(double) * ((size_t)te)*tb);
   posix_memalign((void**) &dataB, 4096, sizeof(double) * ((size_t)te)*tc*ta*td);
   ...
   tcl::Tensor<double> A({tb,te}, dataA);
   tcl::Tensor<double> B({td,tc,te,ta}, dataB);
...
MPI_Barrier(MPI_COMM_WORLD);  double start = omp_get_wtime();
   ...
   auto err = tcl::tensorMult<double>( alpha, A["tb,te"], B["td,tc,te,ta"], beta, C["td,tc,tb,ta"] );
      if( err != tcl::SUCCESS ){
   ...
   RDTSC(btsc);
double flop_count = 2.0*ta*tb*tc*td*te * tot_runtime;
