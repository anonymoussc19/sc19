...
int main(int argc, char** argv)
{
   ...
   posix_memalign((void**) &dataB, 4096, sizeof(double) * ((size_t)te)*ta*tc*td);
   ...
   tcl::Tensor<double> A({te,tb}, dataA);
   tcl::Tensor<double> B({ta,tc,te,td}, dataB);
...
MPI_Barrier(MPI_COMM_WORLD);  double start = omp_get_wtime();
   ...
   auto err = tcl::tensorMult<double>( alpha, A["te,tb"], B["ta,tc,te,td"], beta, C["td,tc,tb,ta"] );
      if( err != tcl::SUCCESS ){
