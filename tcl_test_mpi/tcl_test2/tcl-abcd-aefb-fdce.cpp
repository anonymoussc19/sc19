...
int main(int argc, char** argv)
{
   ...
   tcl::Tensor<double> A({tb,tf,te,ta}, dataA);
   tcl::Tensor<double> B({te,tc,td,tf}, dataB);
...
MPI_Barrier(MPI_COMM_WORLD);  double start = omp_get_wtime();
   ...
   auto err = tcl::tensorMult<double>( alpha, A["tb,tf,te,ta"], B["te,tc,td,tf"], beta, C["td,tc,tb,ta"] );
      if( err != tcl::SUCCESS ){
