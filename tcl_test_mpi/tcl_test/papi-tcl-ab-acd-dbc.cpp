...
int main(int argc, char** argv)
{
    ...
    // handle_error(PAPI_add_event(events,PAPI_PRF_DM));
	// handle_error(PAPI_add_event(events,PAPI_TOT_INS));
	// handle_error(PAPI_add_event(events,PAPI_LST_INS));
   ...
   posix_memalign((void**) &dataA, 4096, sizeof(double) * ((size_t)td)*tc*ta);
   posix_memalign((void**) &dataB, 4096, sizeof(double) * ((size_t)td)*tb*tc);
   posix_memalign((void**) &dataC, 4096, sizeof(double) * ((size_t)ta)*tb);
//   C1 = (double*)memalign( 4096, sizeof(double) * ((size_t)ta)*tb*tc*td);

   // Initialize tensors (data is not owned by the tensors)
   tcl::Tensor<double> A({td,tc,ta}, dataA);
   tcl::Tensor<double> B({tc, tb, td}, dataB);
   tcl::Tensor<double> C({tb,ta}, dataC);
...
double start = omp_get_wtime();
    ...
    for(int ccnt = 0; ccnt < tot_runtime; ccnt++){
        ...
        auto err = tcl::tensorMult<double>( alpha, A["td,tc,ta"], B["tc,tb,td"], beta, C["tb,ta"] );
        if( err != tcl::SUCCESS ){
	...
	printf("event L2 =   %lld\n",values[1]);
	printf("event L3 =   %lld\n",values[2]);
    //     	printf("event PRF_DM =   %lld\n",values[0]);
	// printf("event TOT_INS =   %lld\n",values[1]);
	// printf("event LST_INS =   %lld\n",values[2]);
#endif
