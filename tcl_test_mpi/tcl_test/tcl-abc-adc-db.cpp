...
int main(int argc, char** argv)
{
   ...
   posix_memalign((void**) &dataB, 4096, sizeof(double) * ((size_t)td)*tb);
...
//   C1 = (double*)memalign( 4096, sizeof(double) * ((size_t)ta)*tb*tc*td);
   ...
   tcl::Tensor<double> A({tc,td,ta}, dataA);
   tcl::Tensor<double> B({tb, td}, dataB);
...
MPI_Barrier(MPI_COMM_WORLD);  double start = omp_get_wtime();
   ...
   auto err = tcl::tensorMult<double>( alpha, A["tc,td,ta"], B["tb,td"], beta, C["tc,tb,ta"] );
      if( err != tcl::SUCCESS ){
