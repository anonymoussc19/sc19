...
int main(int argc, char** argv)
{
   ...
   posix_memalign((void**) &dataA, 4096, sizeof(double) * ((size_t)td)*ta*tc);
...
//   C1 = (double*)memalign( 4096, sizeof(double) * ((size_t)ta)*tb*tc*td);
   ...
   tcl::Tensor<double> A({td,ta,tc}, dataA);
   tcl::Tensor<double> B({tb, tc, td}, dataB);
...
MPI_Barrier(MPI_COMM_WORLD);  double start = omp_get_wtime();
   ...
   auto err = tcl::tensorMult<double>( alpha, A["td,ta,tc"], B["tb,tc,td"], beta, C["tb,ta"] );
      if( err != tcl::SUCCESS ){
