...
B[+g* C_range_d* C_range_a* C_range_b+d* C_range_a* C_range_b+a* C_range_b+b] * 
A[+e* C_range_f* C_range_g* C_range_c+f* C_range_g* C_range_c+g* C_range_c+c]; 
}
...
int main(int argc, char** argv)
{
   ...
   posix_memalign((void**) &dataA, 4096, sizeof(double) * ((size_t)te)*tf*tg*tc);
   posix_memalign((void**) &dataB, 4096, sizeof(double) * ((size_t)tg)*td*ta*tb);
   ...
   tcl::Tensor<double> A({tc,tg,tf,te}, dataA);
   tcl::Tensor<double> B({tb,ta,td,tg}, dataB);
...
MPI_Barrier(MPI_COMM_WORLD);  double start = omp_get_wtime();
   ...
   auto err = tcl::tensorMult<double>( alpha, A["tc,tg,tf,te"], B["tb,ta,td,tg"], beta, C["tf,te,td,tc,tb,ta"] );
      if( err != tcl::SUCCESS ){
