...
int main(int argc, char** argv)
{
   ...
   posix_memalign((void**) &dataA, 4096, sizeof(double) * ((size_t)td)*te*tg*tc);
   posix_memalign((void**) &dataB, 4096, sizeof(double) * ((size_t)tg)*tf*ta*tb);
   ...
   tcl::Tensor<double> A({tc,tg,te,td}, dataA);
   tcl::Tensor<double> B({tb,ta,tf,tg}, dataB);
...
MPI_Barrier(MPI_COMM_WORLD);  double start = omp_get_wtime();
   ...
   auto err = tcl::tensorMult<double>( alpha, A["tc,tg,te,td"], B["tb,ta,tf,tg"], beta, C["tf,te,td,tc,tb,ta"] );
      if( err != tcl::SUCCESS ){
