...
B[+g* C_range_e* C_range_b* C_range_c+e* C_range_b* C_range_c+b* C_range_c+c] * 
A[+d* C_range_f* C_range_g* C_range_a+f* C_range_g* C_range_a+g* C_range_a+a]; 
}
...
int main(int argc, char** argv)
{
   ...
   posix_memalign((void**) &dataA, 4096, sizeof(double) * ((size_t)td)*tf*tg*ta);
   posix_memalign((void**) &dataB, 4096, sizeof(double) * ((size_t)tg)*te*tb*tc);
   ...
   tcl::Tensor<double> A({ta,tg,tf,td}, dataA);
   tcl::Tensor<double> B({tc,tb,te,tg}, dataB);
...
MPI_Barrier(MPI_COMM_WORLD);  double start = omp_get_wtime();
   ...
   auto err = tcl::tensorMult<double>( alpha, A["ta,tg,tf,td"], B["tc,tb,te,tg"], beta, C["tf,te,td,tc,tb,ta"] );
      if( err != tcl::SUCCESS ){
