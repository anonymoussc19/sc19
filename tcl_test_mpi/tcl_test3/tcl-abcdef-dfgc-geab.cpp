...
B[+g* C_range_e* C_range_a* C_range_c+e* C_range_a* C_range_c+a* C_range_c+c] * 
A[+d* C_range_f* C_range_g* C_range_b+f* C_range_g* C_range_b+g* C_range_b+b]; 
}
...
int main(int argc, char** argv)
{
   ...
   posix_memalign((void**) &dataA, 4096, sizeof(double) * ((size_t)td)*tf*tg*tc);
   posix_memalign((void**) &dataB, 4096, sizeof(double) * ((size_t)tg)*te*ta*tb);
   ...
   tcl::Tensor<double> A({tc,tg,tf,td}, dataA);
   tcl::Tensor<double> B({tb,ta,te,tg}, dataB);
...
MPI_Barrier(MPI_COMM_WORLD);  double start = omp_get_wtime();
   ...
   auto err = tcl::tensorMult<double>( alpha, A["tb,tg,tf,td"], B["tc,ta,te,tg"], beta, C["tf,te,td,tc,tb,ta"] );
      if( err != tcl::SUCCESS ){
