...
C[+a* C_range_b+b] += 
A[+c* C_range_a* C_range_d+a* C_range_d+d] * 
B[+d* C_range_c* C_range_b+c* C_range_b+b]; 
}
...
A = (double*)memalign(4096, sizeof(double) * A_range_c * A_range_a * A_range_d);
B = (double*)memalign(4096, sizeof(double) * B_range_d * B_range_c * B_range_b);
for(int i = 0; i < 1*A_range_c*A_range_a*A_range_d; i++){
A[i] = rand()%1000;
}
for(int i = 0; i < 1*B_range_d*B_range_c*B_range_b; i++){
B[i] = rand()%1000;
}
...
//Abuf = (double*)memalign(4096, sizeof(double) * A_range_c * A_range_a * A_range_d);
...
int A_stride_c = 1 * A_range_a * A_range_d;
...
//Bbuf = (double*)memalign(4096, sizeof(double) * B_range_d * B_range_c * B_range_b);
...
int B_stride_d = 1 * B_range_c * B_range_b;
int B_stride_c = 1 * B_range_b;
int B_stride_b = 1;
...
Bbuf[B_pack_offset+jj] = B[ +Id*B_stride_d +Ic*B_stride_c +(Ib+jj)*B_stride_b];
}
else{
int Ib = (In - base_n_range) / Ext_B_stride_b;
Bbuf[B_pack_offset] = B[ +Id*B_stride_d +Ic*B_stride_c +Ib*B_stride_b];
}
// end pack body
}
}
...
Abuf[A_pack_offset+ii] = A[ +Ic*A_stride_c +(Ia+ii)*A_stride_a +Id*A_stride_d];
}
else{
int Ia = (Im - base_m_range) / Ext_A_stride_a;
Abuf[A_pack_offset] = A[ +Ic*A_stride_c +Ia*A_stride_a +Id*A_stride_d];
}
// end pack body
}
}
...
// end compute body
    ...
    printf("tot cyc= %12ld,  cycles,    %2.4f, flops/cycle\n",cycles_tuned, flops_cycle_tuned);printf("gflops = %lf", flop_count/1000/1000/1000/runtime); printf("\n");
