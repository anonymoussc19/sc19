...
//    a4=c4=19; f4=d4=;
    ...
    for(int g = 0; g<g4;g++){
        ...
        C[a* b4*c4*d4*e4*f4 +
          ...
          f]  +=
//dega gfbc
            A[e *f4*g4*a4 + f *g4*a4 + g* a4+a *1]
            *
            B[g* d4*b4*c4 + d *b4*c4 + b*c4+ c];
...
//cgab, gefd
    double* data_A = (double*)memalign(4096,e4*f4*g4*a4*sizeof(double));
    tblis_tensor A;


    len_type Altp[4] = {e4,f4,g4,a4}; // m*k
//    stride_type Astp[4] = {g4*a4*b4, a4*b4, b4,1};
    //gabc
    stride_type Astp[4] = {f4*g4*a4 , g4*a4 ,a4 ,1};
    len_type* Alentp = &Altp;
    stride_type*  Asttp = &Astp;
    for(int i=0;i<e4*f4*g4*a4;i++)data_A[i] = rand()/1000;
    tblis_init_tensor_d(&A, 4, Alentp, data_A, Asttp);

    
    double* data_B = (double*)memalign(4096,g4*d4*b4*c4*sizeof(double));
    tblis_tensor B;
    len_type Bltp[4] = {g4,d4,b4,c4 };// n*k
//    stride_type Bstp[4] = {d4*d4*d4, d4*d4, d4,1};
    stride_type Bstp[4] = {b4*d4*c4, b4*c4, c4,1};
    len_type* Blentp = &Bltp;
    stride_type*  Bsttp = &Bstp;
    for(int i=0;i<g4*d4*b4*c4;i++)data_B[i] = rand()/1000;
...
int ccnt = 5;
    ...
    tblis_tensor_mult(tblis_single, NULL, &A, "efga", &B, "gdbc", &C, "abcdef");
//        tblis_tensor_mult(tblis_single, NULL, &A, "cgab", &B, "gefd", &C, "abcdef");
