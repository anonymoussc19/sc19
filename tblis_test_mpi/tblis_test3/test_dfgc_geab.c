...
//    a4=c4=19; f4=d4=;
    ...
    for(int g = 0; g<g4;g++){
        ...
        C[a* b4*c4*d4*e4*f4 +
          ...
          f]  +=
//dega gfbc
            A[d *f4*g4*c4 + f *g4*c4 + g* c4+c *1]
            *
            B[g* e4*a4*b4 + e *a4*b4 + a*b4+ b];
...
//    int m=3072, n=3072, k=3072;
    ...
    double* data_A = (double*)memalign(4096,d4*f4*g4*c4*sizeof(double));
    tblis_tensor A;
    len_type Altp[4] = {d4,f4,g4,c4}; // m*k
    stride_type Astp[4] = {f4*g4*c4 , g4*c4 ,c4 ,1};
    len_type* Alentp = &Altp;
    stride_type*  Asttp = &Astp;
    for(int i=0;i<d4*f4*g4*c4;i++)data_A[i] = rand()/1000;
    ...
    double* data_B = (double*)memalign(4096,g4*e4*a4*b4*sizeof(double));
    tblis_tensor B;
    len_type Bltp[4] = {g4,e4,a4,b4 };// n*k
    stride_type Bstp[4] = {a4*e4*b4, a4*b4, b4,1};
    len_type* Blentp = &Bltp;
    stride_type*  Bsttp = &Bstp;
    for(int i=0;i<g4*e4*a4*b4;i++)data_B[i] = rand()/1000;
...
int ccnt = 5;
    ...
    tblis_tensor_mult(tblis_single, NULL, &A, "dfgc", &B, "geab", &C, "abcdef");
//        tblis_tensor_mult(tblis_single, NULL, &A, "cgab", &B, "gefd", &C, "abcdef");
