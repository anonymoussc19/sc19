...
//    a4=b4=d4=312; c4=24;
    ...
    f4 = C_range_f;
//cgab, gefd
    double* data_A = (double*)memalign(4096,a4*e4*b4*f4*sizeof(double));
    tblis_tensor A;


    len_type Altp[4] = {a4,e4,b4, f4}; // m*k
    stride_type Astp[4] = { e4*b4*f4, b4*f4,  b4, 1};
    len_type* Alentp = &Altp;

    stride_type*  Asttp = &Astp;
    for(int i=0;i<a4*e4*b4*f4;i++)data_A[i] = rand()%1000;
    ...
    tblis_tensor B;
    len_type Bltp[4] = {f4,d4,e4,c4 };// n*k

    
//    stride_type Bstp[4] = {e4*f4*d4, f4*d4, d4,1};
    stride_type Bstp[4] = {d4*c4*e4, c4*e4,e4,1};
    len_type* Blentp = &Bltp;
    stride_type*  Bsttp = &Bstp;
    for(int i=0;i<f4*d4*c4*e4;i++)data_B[i] = rand()%1000;
...
start = omp_get_wtime();
    ...
    tblis_tensor_mult(tblis_single, NULL, &A, "aefb", &B, "fdce", &C, "abcd");
