...
B[+b* C_range_f* C_range_c* C_range_e+f* C_range_c* C_range_e+c* C_range_e+e] * 
A[+a* C_range_e* C_range_d* C_range_f+e* C_range_d* C_range_f+d* C_range_f+f]; 
}
...
//cgab, gefd
    ...
    len_type Altp[4] = {a4,e4,d4, f4}; // m*k
    stride_type Astp[4] = { e4*d4*f4, d4*f4,  f4, 1};
    len_type* Alentp = &Altp;

    stride_type*  Asttp = &Astp;
    for(int i=0;i<a4*e4*d4*f4;i++)data_A[i] = rand()%1000;


    tblis_init_tensor_d(&A, 4, Alentp, data_A, Asttp);

    double* data_B = (double*)memalign(4096,b4*f4*c4*e4*sizeof(double));
    tblis_tensor B;
    len_type Bltp[4] = {b4,f4,d4,e4 };// n*k

    
//    stride_type Bstp[4] = {e4*f4*d4, f4*d4, d4,1};
    stride_type Bstp[4] = {f4*c4*e4, c4*e4,e4,1};
    len_type* Blentp = &Bltp;
    stride_type*  Bsttp = &Bstp;
    for(int i=0;i<b4*f4*c4*e4;i++)data_B[i] = rand()%1000;
...
start = omp_get_wtime();
    ...
    tblis_tensor_mult(tblis_single, NULL, &A, "aedf", &B, "bfce", &C, "abcd");
