...
B[+f* C_range_b* C_range_e* C_range_c+b* C_range_c* C_range_e+e* C_range_c+c] * 
A[+e* C_range_a* C_range_d* C_range_f+a* C_range_d* C_range_f+f* C_range_d+d]; 
}
...
//cgab, gefd
    ...
    stride_type Astp[4] = { a4*d4*f4, d4*f4,  d4, 1};
    ...
    tblis_tensor B;
    len_type Bltp[4] = {f4,b4,e4,c4 };// n*k

    
//    stride_type Bstp[4] = {e4*f4*d4, f4*d4, d4,1};
    stride_type Bstp[4] = {b4*c4*e4, c4*e4,c4,1};
...
start = omp_get_wtime();
    ...
    tblis_tensor_mult(tblis_single, NULL, &A, "eafd", &B, "fbec", &C, "abcd");
