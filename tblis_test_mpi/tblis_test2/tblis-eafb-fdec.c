...
A[+e* C_range_a* C_range_b* C_range_f+a* C_range_b* C_range_f+f* C_range_b+b] * 
...
//cgab, gefd
    ...
    stride_type Astp[4] = { a4*b4*f4, b4*f4,  b4, 1};
...
//    stride_type Bstp[4] = {e4*f4*d4, f4*d4, d4,1};
    stride_type Bstp[4] = {d4*c4*e4, c4*e4,c4,1};
...
start = omp_get_wtime();
    ...
    tblis_tensor_mult(tblis_single, NULL, &A, "eafb", &B, "fdec", &C, "abcd");
