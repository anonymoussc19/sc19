...
//cgab, gefd
    ...
    len_type Altp[2] = { b4,f4}; // m*k
    stride_type Astp[2] = { f4, 1};
    len_type* Alentp = &Altp;

    stride_type*  Asttp = &Astp;
    for(int i=0;i<f4*b4;i++)data_A[i] = rand()%1000;


    tblis_init_tensor_d(&A, 2, Alentp, data_A, Asttp);

    double* data_B = (double*)memalign(4096,e4*f4*c4*a4*d4*sizeof(double));
    tblis_tensor B;
    len_type Bltp[5] = {e4, f4,c4,a4,d4};// n*k

    
//    stride_type Bstp[4] = {e4*f4*d4, f4*d4, d4,1};
    stride_type Bstp[5] = {f4*c4*a4*d4, c4*a4*d4, a4*d4, d4,1};
    len_type* Blentp = &Bltp;
    stride_type*  Bsttp = &Bstp;
    for(int i=0;i<e4*f4*c4*a4*d4;i++)data_B[i] = rand()%1000;
...
start = omp_get_wtime();
    ...
    tblis_tensor_mult(tblis_single, NULL, &A, "bf", &B, "efcad", &C, "abcde");
//        tblis_tensor_mult(tblis_single, NULL, &A, "cgab", &B, "gefd", &C, "abcdef");
