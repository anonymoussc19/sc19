...
//    a4=b4=d4=312; c4=24;
    ...
    d4 = C_range_d;
//cgab, gefd
    double* data_A = (double*)memalign(4096,a4*c4*d4*sizeof(double));
    tblis_tensor A;


    len_type Altp[3] = { c4,a4,d4}; // m*k
    stride_type Astp[3] = { a4*d4,  d4, 1};
    len_type* Alentp = &Altp;

    stride_type*  Asttp = &Astp;
    for(int i=0;i<d4*a4*c4;i++)data_A[i] = rand()%1000;


    tblis_init_tensor_d(&A, 3, Alentp, data_A, Asttp);

    double* data_B = (double*)memalign(4096,b4*c4*d4*sizeof(double));
    tblis_tensor B;
    len_type Bltp[3] = {d4,c4, b4 };// n*k

    
//    stride_type Bstp[4] = {e4*f4*d4, f4*d4, d4,1};
    stride_type Bstp[3] = {c4*b4,b4,1};
    len_type* Blentp = &Bltp;
    stride_type*  Bsttp = &Bstp;
    for(int i=0;i<b4*c4*d4;i++)data_B[i] = rand()%1000;
    tblis_init_tensor_d(&B, 3, Blentp, data_B, Bsttp);

    double* data_C = (double*)memalign(4096,a4*b4*sizeof(double));
    double* data_C2 = (double*)memalign(4096,a4*b4*sizeof(double));
    for(int i = 0; i < a4*b4; i++){
        data_C[i] = data_C2[i]=0;
    }

    tblis_tensor C;
    len_type Cltp[2] = {a4,b4}; // m*n
//    stride_type Cstp[6] = {24*24*24*24*24, 24*24*24*24,24*24*24,24*24,24,1};
    stride_type Cstp[2] = {b4, 1};
    len_type* Clentp = &Cltp;
    stride_type*  Csttp =&Cstp;

    tblis_init_tensor_d(&C, 2, Clentp, data_C, Csttp);
...
start = omp_get_wtime();
    ...
    tblis_tensor_mult(tblis_single, NULL, &A, "cad", &B, "dcb", &C, "ab");
//        tblis_tensor_mult(tblis_single, NULL, &A, "cgab", &B, "gefd", &C, "abcdef");
