...
C[+a* C_range_c* C_range_b+c* C_range_b+b] += 
B[+d* C_range_c+c] * 
...
//    a4=b4=d4=312; c4=24;
    ...
    d4 = A_range_d;
//cgab, gefd
    double* data_A = (double*)memalign(4096,a4*b4*d4*sizeof(double));
    tblis_tensor A;


    len_type Altp[3] = {b4,d4, a4}; // m*k
    stride_type Astp[3] = { d4*a4,  a4, 1};
    len_type* Alentp = &Altp;

    stride_type*  Asttp = &Astp;
    for(int i=0;i<a4*b4*d4;i++)data_A[i] = rand()%1000;


    tblis_init_tensor_d(&A, 3, Alentp, data_A, Asttp);

    double* data_B = (double*)memalign(4096,d4*c4*sizeof(double));
    tblis_tensor B;
    len_type Bltp[2] = {d4,c4 };// n*k

    
//    stride_type Bstp[4] = {e4*f4*d4, f4*d4, d4,1};
    stride_type Bstp[2] = {c4,1};
    len_type* Blentp = &Bltp;
    stride_type*  Bsttp = &Bstp;
    for(int i=0;i<d4*c4;i++)data_B[i] = rand()%1000;
    tblis_init_tensor_d(&B, 2, Blentp, data_B, Bsttp);
...
start = omp_get_wtime();
    ...
    tblis_tensor_mult(tblis_single, NULL, &A, "bda", &B, "dc", &C, "acb");
//        tblis_tensor_mult(tblis_single, NULL, &A, "cgab", &B, "gefd", &C, "abcdef");
