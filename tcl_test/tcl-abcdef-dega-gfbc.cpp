...
int main(int argc, char** argv)
{
   ...
   posix_memalign((void**) &dataA, 4096, sizeof(double) * ((size_t)td)*te*tg*ta);
   posix_memalign((void**) &dataB, 4096, sizeof(double) * ((size_t)tg)*tf*tb*tc);
   ...
   tcl::Tensor<double> A({ta,tg,te,td}, dataA);
   tcl::Tensor<double> B({tc,tb,tf,tg}, dataB);
...
//#pragma omp parallel for
 ...
 /* warm up */
   ...
   auto err = tcl::tensorMult<double>( alpha, A["ta,tg,te,td"], B["tc,tb,tf,tg"], beta, C["tf,te,td,tc,tb,ta"] );
      if( err != tcl::SUCCESS ){
