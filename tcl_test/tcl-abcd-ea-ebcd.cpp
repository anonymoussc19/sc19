...
int main(int argc, char** argv)
{
   ...
   posix_memalign((void**) &dataA, 4096, sizeof(double) * ((size_t)te)*ta);
   posix_memalign((void**) &dataB, 4096, sizeof(double) * ((size_t)te)*tc*tb*td);
   ...
   tcl::Tensor<double> A({ta,te}, dataA);
   tcl::Tensor<double> B({td,tc,tb,te}, dataB);
...
//#pragma omp parallel for
 ...
 /* warm up */
   ...
   auto err = tcl::tensorMult<double>( alpha, A["ta,te"], B["td,tc,tb,te"], beta, C["td,tc,tb,ta"] );
      if( err != tcl::SUCCESS ){
