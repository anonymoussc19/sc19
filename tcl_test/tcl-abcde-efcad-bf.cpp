...
int main(int argc, char** argv)
{
   ...
   posix_memalign((void**) &dataA, 4096, sizeof(double) * ((size_t)td)*ta*tc*tf*te);
   posix_memalign((void**) &dataB, 4096, sizeof(double) * ((size_t)tf)*tb);
...
//   C1 = (double*)memalign( 4096, sizeof(double) * ((size_t)ta)*tb*tc*td);
   ...
   tcl::Tensor<double> A({td,ta,tc,tf,te}, dataA);
   tcl::Tensor<double> B({tf,tb}, dataB);
...
//#pragma omp parallel for
 ...
 /* warm up */
   ...
   auto err = tcl::tensorMult<double>( alpha, A["td,ta,tc,tf,te"], B["tf,tb"], beta, C["te,td,tc,tb,ta"] );
      if( err != tcl::SUCCESS ){
