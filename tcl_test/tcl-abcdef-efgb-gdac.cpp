...
B[+g* C_range_d* C_range_a* C_range_c+d* C_range_a* C_range_c+a* C_range_c+c] * 
A[+e* C_range_f* C_range_g* C_range_b+f* C_range_g* C_range_b+g* C_range_b+b]; 
}
...
int main(int argc, char** argv)
{
   ...
   posix_memalign((void**) &dataA, 4096, sizeof(double) * ((size_t)te)*tf*tg*tb);
   posix_memalign((void**) &dataB, 4096, sizeof(double) * ((size_t)tg)*td*ta*tc);
   ...
   tcl::Tensor<double> A({tb,tg,tf,te}, dataA);
   tcl::Tensor<double> B({tc,ta,td,tg}, dataB);
...
//#pragma omp parallel for
 ...
 /* warm up */
   ...
   auto err = tcl::tensorMult<double>( alpha, A["tb,tg,tf,te"], B["tc,ta,td,tg"], beta, C["tf,te,td,tc,tb,ta"] );
      if( err != tcl::SUCCESS ){
