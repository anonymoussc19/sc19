...
int main(int argc, char** argv)
{
   ...
   posix_memalign((void**) &dataA, 4096, sizeof(double) * ((size_t)td)*ta);
   posix_memalign((void**) &dataB, 4096, sizeof(double) * ((size_t)td)*tc*tb);
...
//   C1 = (double*)memalign( 4096, sizeof(double) * ((size_t)ta)*tb*tc*td);
   ...
   tcl::Tensor<double> A({td,ta}, dataA);
   tcl::Tensor<double> B({tc,td, tb}, dataB);
...
//#pragma omp parallel for
 ...
 /* warm up */
   ...
   auto err = tcl::tensorMult<double>( alpha, A["td,ta"], B["tc,td,tb"], beta, C["tc,tb,ta"] );
      if( err != tcl::SUCCESS ){
