...
int main(int argc, char** argv)
{
   ...
   posix_memalign((void**) &dataA, 4096, sizeof(double) * ((size_t)td)*te*tg*tb);
   posix_memalign((void**) &dataB, 4096, sizeof(double) * ((size_t)tg)*tf*ta*tc);
   ...
   tcl::Tensor<double> A({tb,tg,te,td}, dataA);
   tcl::Tensor<double> B({tc,ta,tf,tg}, dataB);
...
//#pragma omp parallel for
 ...
 /* warm up */
   ...
   auto err = tcl::tensorMult<double>( alpha, A["tb,tg,te,td"], B["tc,ta,tf,tg"], beta, C["tf,te,td,tc,tb,ta"] );
      if( err != tcl::SUCCESS ){
