...
int main(int argc, char** argv)
{
   ...
   posix_memalign((void**) &dataA, 4096, sizeof(double) * ((size_t)te)*tc);
   posix_memalign((void**) &dataB, 4096, sizeof(double) * ((size_t)te)*ta*tb*td);
   ...
   tcl::Tensor<double> A({tc,te}, dataA);
   tcl::Tensor<double> B({td,te,tb,ta}, dataB);
...
//#pragma omp parallel for
 ...
 /* warm up */
   ...
   auto err = tcl::tensorMult<double>( alpha, A["tc,te"], B["td,te,tb,ta"], beta, C["td,tc,tb,ta"] );
      if( err != tcl::SUCCESS ){
