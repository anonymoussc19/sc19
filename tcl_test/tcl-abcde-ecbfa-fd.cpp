...
int main(int argc, char** argv)
{
   ...
   posix_memalign((void**) &dataA, 4096, sizeof(double) * ((size_t)tc)*ta*tb*tf*te);
   posix_memalign((void**) &dataB, 4096, sizeof(double) * ((size_t)tf)*td);
...
//   C1 = (double*)memalign( 4096, sizeof(double) * ((size_t)ta)*tb*tc*td);
   ...
   tcl::Tensor<double> A({ta,tf,tb,tc,te}, dataA);
   tcl::Tensor<double> B({td,tf}, dataB);
...
//#pragma omp parallel for
 ...
 /* warm up */
   ...
   auto err = tcl::tensorMult<double>( alpha, A["ta,tf,tb,tc,te"], B["td,tf"], beta, C["te,td,tc,tb,ta"] );
      if( err != tcl::SUCCESS ){
