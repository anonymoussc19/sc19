...
int main(int argc, char** argv)
{
   ...
   posix_memalign((void**) &dataB, 4096, sizeof(double) * ((size_t)td)*te*tb*tf);
   ...
   tcl::Tensor<double> A({tf,tc,te,ta}, dataA);
   tcl::Tensor<double> B({td,te,tb,tf}, dataB);
...
//#pragma omp parallel for
 ...
 /* warm up */
   ...
   auto err = tcl::tensorMult<double>( alpha, A["tf,tc,te,ta"], B["td,te,tb,tf"], beta, C["td,tc,tb,ta"] );
      if( err != tcl::SUCCESS ){
